...
##### OpenCV (highly recommended, version >= 3.4)
...
##### Pangolin (highly recommended, version >= 0.2).
...
this will compile a library `libdso.dylib`, which can be linked from external projects.
...
- `images_path` : A folder containing images. They are sorted *alphabetically*.
camera_calib_path:
- `gamma_calib_path`: A gamma calibration file, containing a single row with 256 values, mapping [0..255] to the respective irradiance value, i.e. containing the *discretized inverse response function*. See TUM monoVO dataset for an example.

- `vignette_path`: A monochrome 16bit or 8bit image containing the vignette as pixelwise attenuation factors. See TUM monoVO dataset for an example.

- `camera_calib_path`: A geometric camera calibration file. See below.
